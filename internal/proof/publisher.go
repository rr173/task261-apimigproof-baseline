...
func (p *Publisher) Supersede(oldID, newID int64) (model.MigrationProof, error) {
	oldProof, err := p.store.GetProof(oldID)
	if err != nil {
		return model.MigrationProof{}, err
	}
	newProof, err := p.store.GetProof(newID)
	if err != nil {
		return model.MigrationProof{}, err
	}
	if newProof.Status != model.ProofPublished {
		return model.MigrationProof{}, fmt.Errorf("%w: replacement proof %d is not published", model.ErrProofSealed, newID)
	}
	if oldProof.FromContractID != newProof.FromContractID || oldProof.ToContractID != newProof.ToContractID {
		return model.MigrationProof{}, fmt.Errorf("%w: proofs belong to different contract pairs", model.ErrConflict)
	}
